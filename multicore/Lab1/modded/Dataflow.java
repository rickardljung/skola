...
class Vertex {
	...
	public static void liveness(Vertex vertex[])
	{
		...
		while (!worklist.isEmpty()) {
			...
			//System.out.println(i++ + ". worklist: " + worklist.toString());
		}
		end = System.nanoTime();
	...
	public static void parLiveness(final Vertex vertex[], final int nthread)
	{
		Vertex			v;
		long			begin;
		long			end;
		...
		final int nn = n;
			t[n] = new Thread(new Runnable() {
				@Override
				public void run() {
					Vertex			u;
					LinkedList<Vertex>	worklist = new LinkedList<Vertex>();

					for (int i = 0; i < vertex.length/nthread * (nn + 1); ++i) {
						worklist.addLast(vertex[i]);
						vertex[i].listed = true;
					}
